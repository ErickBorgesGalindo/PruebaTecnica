        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 40%;">Secciones</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $profile->code }}</td>
                    <td>{{ $profile->name }}</td>
                    <td>{{ is_array($profile->sections) ? implode(', ', $profile->sections) : $profile->sections }}</td>
                    <td>{{ $profile->created_at ? $profile->created_at->format('d/m/Y H:i') : '-' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        <div class="summary">Total: {{ count($profiles) }} perfiles</div>
    </div>
